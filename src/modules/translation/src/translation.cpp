...
int build_translation_context(struct translation_context &translation_ctx)
{
	...
	Logger::log(Logger::Level::INFO, "Building translation context from '%s'...",
		    local_model_path.c_str());
	...
	try {
		Logger::log(Logger::Level::INFO, "Loading SPM from %s", local_spm_path.c_str());
		...
		if (!target_spm_path.empty()) {
			Logger::log(Logger::Level::INFO, "Loading target SPM from %s",
				    target_spm_path.c_str());
			...
			Logger::log(Logger::Level::INFO,
				    "Target SPM not found, using source SPM for target");
	      ...
	      const std::string &source_lang, const std::string &target_lang, std::string &result)
{
	try {
		...
		if (translation_ctx.input_tokenization_style == INPUT_TOKENIZAION_M2M100) {
			...
			Logger::log(Logger::Level::INFO, "Input tokens: %s",
			...
			Logger::log(Logger::Level::INFO, "Target prefix: %s",
		...
		Logger::log(Logger::Level::INFO, "Translation tokens: %s",
		...
		Logger::log(Logger::Level::INFO, "Last translation tokens deque size: %d",
