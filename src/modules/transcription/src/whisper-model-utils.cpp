...
void update_whisper_model(struct transcription_context *gf)
{
	if (gf->context == nullptr) {
		Logger::log(Logger::Level::ERROR, "obs_source_t context is null");
		return;
	}

	obs_data_t *s = obs_source_get_settings(gf->context);
	if (s == nullptr) {
		Logger::log(Logger::Level::ERROR, "obs_data_t settings is null");
		return;
	}
	...
	if (!is_external_model && new_model_path.empty()) {
		Logger::log(Logger::Level::WARNING, "Whisper model path is empty");
		return;
	}
	if (is_external_model && external_model_file_path.empty()) {
		Logger::log(Logger::Level::WARNING, "External model file path is empty");
		return;
	}

	char *silero_vad_model_file = obs_module_file("models/silero-vad/silero_vad.onnx");
	if (silero_vad_model_file == nullptr) {
		Logger::log(Logger::Level::ERROR, "Cannot find Silero VAD model file");
		return;
	}
		...
		if (!is_external_model) {
			...
			if (external_model_file_path.empty()) {
				Logger::log(Logger::Level::WARNING,
					    "External model file path is empty");
			} else {
				...
				if (gf->whisper_model_file_currently_loaded ==
					...
					Logger::log(Logger::Level::INFO,
						    "External model file is already loaded");
					return;
				} else {
					shutdown_whisper_thread(gf);
					gf->whisper_model_path = new_model_path;
	...
	if (new_dtw_timestamps != gf->enable_token_ts_dtw) {
		...
		Logger::log(gf->log_level, "dtw_token_timestamps changed from %d to %d",
